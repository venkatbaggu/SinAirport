...
{
	CameraManager::instance()->m_data.lock();

	std::async(boost::bind(
		&CameraManager::write,
		CameraManager::instance(),
		cv::Mat(pFrame->nHeight + pFrame->nHeight / 2,
		pFrame->nWidth, CV_8UC1, pBuf),
		nPort
		));

	//std::cout << nPort << std::endl;

	CameraManager::instance()->m_data.unlock();
